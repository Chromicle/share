...
public class FormsAdapter extends CursorRecyclerViewAdapter<FormsAdapter.FormHolder> {
    ...
    @Nullable private final LinkedHashSet<Long> selectedForms;
    ...
    public FormsAdapter(
            ...
            Cursor cursor,
            ItemClickListener listener,
            @Nullable LinkedHashSet<Long> selectedForms,
            InstancesDao instancesDao,
            TransferDao transferDao) {
    ...
    public void onBindViewHolder(FormHolder viewHolder, Cursor cursor) {
        ...
        viewHolder.checkBox.setChecked(
                selectedForms != null && selectedForms.contains(((long) form.getId())));
        ...
        if (selectedForms == null) {
            ...
            try (Cursor instanceCursor = instancesDao.getInstancesCursor(selection, selectionArgs)) {
                ...
                if (instanceCursor.getCount() > 0) {
                    ...
                    while (instanceCursor.moveToNext()) {
                        ...
                        selectionArgs[i++] =
                                String.valueOf(
                                        instanceCursor.getLong(
                                                instanceCursor.getColumnIndex(InstanceProviderAPI.InstanceColumns._ID)));
                    }
                }
                ...
                try (Cursor transferCursor =
                        transferDao.getInstancesCursor(null, selection, selectionArgs, null)) {
                    if (transferCursor != null && transferCursor.getCount() > 0) {
                        transferCursor.moveToPosition(-1);
                        while (transferCursor.moveToNext()) {
                            long status =
                                    transferCursor.getLong(
                                            transferCursor.getColumnIndex(TransferInstance.REVIEW_STATUS));
                            if (status == TransferInstance.STATUS_ACCEPTED
                                    || status == TransferInstance.STATUS_REJECTED) {
                                reviewed++;
                            } else {
                                unreviewed++;
                            }
            ...
            viewHolder.reviewedForms.setText(
                    context.getString(R.string.num_reviewed, String.valueOf(reviewed)));
            viewHolder.unReviewedForms.setText(
                    context.getString(R.string.num_unreviewed, String.valueOf(unreviewed)));
        } else {
